...
## Метод PUSH
 После внесения изменеий в локальную версию проекта нам потребуется их опубликовать. Для этого используется метод *push*
 >**git push** – отправляет локальную версию репозитория на внешний  
 
 
 Теперь мы готовы предложить наши изменения владельцу проекта. Для этого используется метод *pull request* (в GitHub реализован кнопкой в интерфейсе репозитория).
 Так же отметим, что после зоздания запроса *pull request* все опубликованные методом *push* изменения репозитория будут автоматически переданы в этот запрос.